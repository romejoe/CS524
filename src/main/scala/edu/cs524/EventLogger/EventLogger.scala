...
object EventLogger {
  ...
  object Tabulator {
    def format(table: Seq[Seq[Any]]) = table match {
      ...
      case _ =>
        ...
        val colSizes = (for (col <- sizes.transpose) yield col.max).map(_ + 5)
  ...
  def CollectResults(DisplayOldFormat: Boolean = false) = {
    ...
    val avg = Averages.mapValues("%.2f".format(_))
    val med = Medians.mapValues(_.toString)
    val max = Maxs.mapValues(_.toString)
    val min = Mins.mapValues(_.toString)
